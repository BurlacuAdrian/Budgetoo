...
async function getPayloadWithFamilyData(_id, email, year, month, userCurrency, defaultBudget = 3000, nickname) {
  const payload = {
...
app.get('/v1/start-data/:year/:month', async (req, res, next) => {
  try {
    ...
    const payload = await getPayloadWithFamilyData(_id, email, year, month, user.currency, null ,user.nickname);
...
app.get('/v1/transactions/:year/:month', async (req, res, next) => {
  try {
    ...
    const payload = await getPayloadWithFamilyData(_id, email, year, month, "EUR"); // Default to EUR for transactions endpoint
    return res.status(200).json(payload);
  } catch (error) {
